...
class ArchiveSettings:
    """Settings that control raw-session archive cadence."""
...
def load_archive_settings(path: Path = DEFAULT_CONFIG_PATH) -> ArchiveSettings:
    ...
    if env_value is not None:
        try:
            ...
            raw = {}
        return ArchiveSettings(interval_user_turns=env_value, projects=_projects(raw))

    try:
        raw = tomllib.loads(path.read_text(encoding="utf-8"))
        config_value = _positive_integer(raw.get("archive", {}).get("interval_user_turns"))
        if config_value is not None:
            return ArchiveSettings(interval_user_turns=config_value, projects=_projects(raw))
        return ArchiveSettings(projects=_projects(raw))
